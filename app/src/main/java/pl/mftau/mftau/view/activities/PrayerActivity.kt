...
class PrayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        prayerList.adapter = ArrayAdapter<String>(this@PrayerActivity,
                android.R.layout.simple_list_item_1, mPrayerNames)

        prayerList.setOnItemClickListener { parent, _, position, _ ->
            isTextShowed = true
            prayerText.text = mPrayerBook[position]
    ...
    override fun onBackPressed() {
        if (!isTextShowed) NavUtils.navigateUpFromSameTask(this@PrayerActivity)
        else {
            isTextShowed = false
    ...
    private val mPrayerBook = arrayOf(
    ...
    private val mPrayerNames = arrayOf(
